          ...
          {providers.map((provider) => {
            return (
              <button
                ...
                className="data-[active=true]:bg-muted/50 border-tertiary-dark flex flex-1 flex-col justify-center gap-1 border-t px-6 py-4 even:border-l md:border-l md:border-t-0"
                onClick={() => setActiveChart(provider)}
              >
                <span className="text-muted-foreground text-xs">{provider.replace("https://", "")}</span>
                <span className="text-lg font-bold leading-none md:text-3xl">
